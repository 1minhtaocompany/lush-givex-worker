...
ALLOWED_STATES_STR = ", ".join(ALLOWED_STATES)
...
def add_new_state(state_name: str) -> State:
    ...
    if state_name not in ALLOWED_STATES:
        raise ValueError(
             f"state_name '{state_name}' is not allowed. Allowed states: {ALLOWED_STATES_STR}"
        )
    with _states_lock:
...
def reset_states() -> None:
    """Reset all internal states to initial empty set."""
    global _states
    with _states_lock:
        _states.clear()