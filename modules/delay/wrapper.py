...
def inject_step_delay(
    ...
    Click: real sleep, NOT accumulated. Typing/thinking: accumulated against step ceiling.
    Returns 0.0 when no delay was injected.
    """
    if action_type == "click":
        delay = engine.calculate_click_delay()
        if delay > 0:
            _log.debug("inject_step_delay: action=click injected=%.4fs (not accumulated)", delay)
    ...
    _log.debug(
        ...
        action_type, delay, engine.get_step_accumulated_delay(),
    )
    return delay
...
def wrap(task_fn, persona: PersonaProfile, stop_event: threading.Event | None = None):
    ...
    Injects typing delay before task_fn and thinking delay after (Blueprint §4, §5).
    ...
    _log.debug("wrap: starting cycle persona_type=%s seed=%d", persona.persona_type, persona._seed)
