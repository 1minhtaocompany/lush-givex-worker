...
| **Đầu ra (Deliverables)** | **File:** `modules/delay/biometrics.py` (≤200 dòng). **Class/API:** `BiometricProfile(persona: PersonaProfile)` với methods: `generate_keystroke_delay(char_index: int) → float` (inter-keystroke delay, log-normal distribution, clamped), `generate_burst_pattern(total_chars: int) → list[float]` (danh sách delay cho mỗi ký tự, có burst rhythm), `generate_4x4_pattern() → list[float]` (16 delay values cho 16 số thẻ theo quy tắc 4×4: 4 nhanh → khựng → 4 nhanh → khựng), `apply_noise(base_delay: float) → float` (gaussian noise ±10%). **Tests:** `tests/test_biometrics.py` — distribution validation, burst pattern structure, 4×4 pattern compliance, deterministic output, clamp enforcement. |
