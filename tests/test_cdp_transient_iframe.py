...
def _make_fast_monitor_class():
    """Return a TransientMonitor subclass that polls every 0.05 s (test speed-up).

    The forced interval replaces whatever the caller would have passed so
    functional tests complete in well under a second.
    """
    from modules.monitor.main import TransientMonitor

    class _FastMonitor(TransientMonitor):
        def __init__(self, detector, interval=0.5, on_detect=None):  # noqa: ARG002 - interval intentionally overridden for tests
...
class TestSubmitPurchaseVBVDetectionFunctional(unittest.TestCase):
    ...
    def test_real_monitor_detects_vbv_during_submit(self):
        ...
        from modules.monitor.main import get_metrics
        ...
        with patch("modules.monitor.main.TransientMonitor", _make_fast_monitor_class()):
            gd.submit_purchase()
